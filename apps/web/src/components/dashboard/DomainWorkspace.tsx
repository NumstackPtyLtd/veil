...
export function DomainWorkspace({ engineUrl, token, org, domain, onNavigateUp }: Props) {
    ...
    <div className="max-w-[1280px] mx-auto px-6 py-4 pb-16 text-[13px]">
      <div className="py-2">
        <nav className="flex items-center gap-1.5 text-[11px]" aria-label="Breadcrumb">
          <button onClick={onNavigateUp} className="transition-opacity hover:opacity-80" style={{ color: 'var(--text-muted)' }}>
            {org.name}
          </button>
          <span style={{ color: 'var(--text-muted)' }}>/</span>
          <button onClick={() => setSection('overview')} className="transition-opacity hover:opacity-80" style={{ color: 'var(--text-muted)' }}>
            {domain.name}
          </button>
          <span style={{ color: 'var(--text-muted)' }}>/</span>
      ...
      <div className="flex flex-col md:flex-row" style={{ minHeight: 'calc(100vh - 140px)' }}>
        <VeilNav section={section} onSectionChange={setSection} />
        <div className="flex-1 min-w-0 md:pl-6 py-3">
          {section === 'overview' && <Overview engineUrl={engineUrl} token={token} orgId={org.id} domainId={domain.id} tenantId={domain.salt} />}
          {section === 'sandbox' && <Sandbox engineUrl={engineUrl} tenantId={domain.salt} />}
          {section === 'activity' && <ActivityView engineUrl={engineUrl} token={token} orgId={org.id} domainId={domain.id} />}
